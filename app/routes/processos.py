...
def criar_processo():
    ...
    try:
        ...
        campos_obrigatorios = ['numero_processo', 'titulo', 'area_juridica', 'cliente_id', 'advogado_id']
        ...
        processo = Processo(
            ...
            data_distribuicao=datetime.fromisoformat(data['data_distribuicao']) if data.get('data_distribuicao') else None,
    ...
    try:
        ...
        for andamento in processo.andamentos:
            andamentos_data.append({
                ...
                } if andamento.usuario else None,
        ...
        return jsonify({
            ...
            'juiz': processo.juiz,
            'valor_causa': float(processo.valor_causa) if processo.valor_causa else None,
            'valor_honorarios': float(processo.valor_honorarios) if processo.valor_honorarios else None,
...
def atualizar_processo(processo_id):
    ...
    try:
        ...
        if ('numero_processo' in data and 
        ...
        campos_atualizaveis = [
            'numero_processo', 'numero_interno', 'titulo', 'descricao',
            'area_juridica', 'tipo_acao', 'status', 'tribunal', 'vara',
            'juiz', 'valor_causa', 'valor_honorarios', 'forma_pagamento',
            'prioridade', 'observacoes', 'observacoes_internas',
            'cliente_id', 'advogado_id'
        ]
        ...
        if 'data_distribuicao' in data:
            processo.data_distribuicao = datetime.fromisoformat(data['data_distribuicao']) if data['data_distribuicao'] else None
        
        if 'data_conclusao' in data:
            processo.data_conclusao = datetime.fromisoformat(data['data_conclusao']) if data['data_conclusao'] else None
...
def listar_andamentos(processo_id):
    ...
    try:
        ...
        andamentos_query = Andamento.query.filter_by(processo_id=processo_id).order_by(Andamento.data_andamento.desc())
...
def criar_andamento(processo_id):
    ...
    try:
        ...
        if not data.get('tipo_andamento') or not data.get('descricao'):
            return jsonify({'erro': 'Tipo e descrição do andamento são obrigatórios'}), 400
        
        # Cria novo andamento
        andamento = Andamento(
            data_andamento=datetime.fromisoformat(data['data_andamento']) if data.get('data_andamento') else datetime.utcnow(),
