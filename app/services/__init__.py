...
class DashboardService:
    ...
    def get_estatisticas_gerais():
        ...
        processos_por_area = db.session.query(
            Processo.area_juridica,
            func.count(Processo.id).label('total')
        ).group_by(Processo.area_juridica).all()
        
        # Processos por prioridade
        processos_por_prioridade = db.session.query(
            Processo.prioridade,
            func.count(Processo.id).label('total')
        ).group_by(Processo.prioridade).all()
        ...
        return {
            ...
            'processos_por_status': [{'status': r.status, 'total': r.total} for r in processos_por_status],
            'processos_por_area': [{'area': r.area_juridica, 'total': r.total} for r in processos_por_area],
            'processos_por_prioridade': [{'prioridade': r.prioridade, 'total': r.total} for r in processos_por_prioridade]
        }
    ...
    def get_processos_recentes(limite=10):
        ...
        processos = Processo.query.join(Cliente).join(Advogado)\
    ...
    def get_advogados_produtividade():
        ...
        resultado = db.session.query(
         ...
         .filter(Advogado.ativo == True)\
         .group_by(Advogado.id, Advogado.nome, Advogado.oab_numero, Advogado.oab_estado)\
         .order_by(func.count(Processo.id).desc()).all()
...
class RelatorioService:
    ...
    def processos_por_periodo(data_inicio, data_fim):
        ...
        processos = Processo.query.filter(
            and_(
                Processo.created_at >= data_inicio,
                Processo.created_at <= data_fim
            )
        ).join(Cliente).join(Advogado).all()
    ...
    def clientes_sem_processos():
        ...
        clientes = Cliente.query.filter(
            and_(
                Cliente.ativo == True,
                ~Cliente.id.in_(
                    db.session.query(Processo.cliente_id).distinct()
                )
            )
        ).all()
